...
class ToReadList extends React.Component {
  ...
  fillUpdateForm = (event) => {
    setTimeout(() => {
      for(let i=0; i<this.props.list.length; i++) {
        if(parseInt(this.state.selectedBookId) === parseInt(this.props.list[i].id)) {
          this.setState({
            selectedBookTitle: this.props.list[i].name,
            selectedBookAuthor: this.props.list[i].author,
            selectedBookGenre: this.props.list[i].genre
          })
        }
      }
    }, 10)
  ...
  updateBook = (event) => {
    event.preventDefault()
    const url = `https://dry-meadow-55679.herokuapp.com/booksunread/${this.state.selectedBookId}`
    const postData = {
  ...
  deleteBook = (event) => {
    ...
    .then(response => response.json())
    .then(booksUnread => this.props.fetchBooksUnread(booksUnread))
    .then(this.resetForm())
  }
